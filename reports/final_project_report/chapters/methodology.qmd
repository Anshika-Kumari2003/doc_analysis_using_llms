...
def generate_sql_with_ollama(query, table_name, columns, model=OLLAMA_MODEL):
    ...
    prompt = f"""
You are an expert SQL query generator. Given a natural language question and 
database schema, generate a precise SQL query.
...
7. If a column name in the question doesn't exist, find the closest matching 
   column from the list above
"""
```
