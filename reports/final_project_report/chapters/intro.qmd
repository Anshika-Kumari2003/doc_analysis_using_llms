...
This project introduces a **multi-modal AI system** designed to handle different data types through four intelligent modules:
...
2. **Document Analysis via RAG (Retrieval-Augmented Generation):**  
   This module allows users to upload PDF or TXT documents, which are semantically chunked and embedded into a vector database using Pinecone. The system implements company-specific document processing for EnerSys, Apple, and NVIDIA, with intelligent page-level citation and visual reference support. Leveraging **Phi-3 Mini**, a lightweight LLM, users can ask context-sensitive questions and receive accurate answers based on retrieved document chunks [@lewis2020rag]. The implementation includes comprehensive error handling and fallback mechanisms for robust performance.

3. **YouTube Transcript Agent:**  
   Using state-of-the-art speech recognition models like **Youtube Transcript API** [@radford2023whisper], this module transcribes YouTube videos, summarizes the content, and answers follow-up questions through a GUI-powered interface. The system features a persistent caching mechanism for transcripts, background processing capabilities, and support for multiple LLM models (Phi-3 Mini, Mistral, Llama3, Gemma). It offers a valuable solution for educational or research-based video analysis [@rao2023yt], with robust error handling for unavailable transcripts.

4. **SQL Agent for Structured Data (Excel):**  
   Users can upload Excel sheets containing structured data, and the system converts natural language questions into executable SQL queries. The implementation includes intelligent fuzzy column matching, support for multiple CSV encodings (UTF-8, Latin-1, CP1252, ISO-8859-1), and a dual approach combining Ollama-based generation with rule-based fallback. This allows for intuitive querying without needing manual database interaction, drawing on advances in text-to-SQL modeling [@yu2018spider; @shi2022text2sql].

By combining document retrieval, audio processing, and structured data querying, this system demonstrates how modern LLMs can bridge the gap between raw data and human-centric insights across domains. The implementation features a unified Gradio interface with specialized tabs for each module, making it accessible to users without technical expertise.
...
- A **Multi-Agent Assistant** that automatically analyzes user queries and routes them to the most suitable agent—whether it involves company-specific data, video searches, or web-based information retrieval—creating a unified conversational interface.
